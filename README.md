...
框架的功能扩充使用了插件系统，用户添加的功能为模块，有四种模块类型，框架使用扫描和反射的方法统一进行生命周期管理，`studio.blacktech.furryblackplus.demo`包含示例模块代码，模块必须继承对应的模块类并填写注解。
...
- 关闭顺序为：机器人登出，关闭消息接收，执行器，监听器，过滤器，定时器；
...
- 重构 将模块相关的功能转移到Schema类
- 恢复四种注解

#### 烂代码

- 还是没有实现reload 而且即使实现也不能做到完美

### REVERT to 0.8.0-SNAPSHOT

#### 一个悖论

前提：构造函数调用super之前不可以调用this  
影响：final/避免继承构造二选一

spring bean scan的解决办法是：

- 要么使用final+构造
- 要么使用Autowire不用final

FurryBlack的解决办法：   
提升SDK使用体验，避免开发时需要继承构造方法

使用违法反射调用可以干掉final修饰符，就算使用违法反射在运行时添加final，也不可阻止setter方法，故放弃final写法

#### 成功

- 取消github-CI，没用
- 模块取舍的改回了使用构造+final的设计，但是本质是构造时扫描注解，插件包来源信息无法包含，只能外部添加关联(又多了一个RDMS的理由)
- 插件reload不一定保证模块列表相同，涉及到覆盖，其操作逻辑复杂(又多了一个RDMS的理由)
- 目前仅能实现卸载，不能实现加载，而且由于不存在ConcurrentLinkedHashMap所以卸载线程不安全(又多了一个RDMS的理由)

**预计下个版本咋都得改成RDMS-like的插件和模块管理机制**
...
- 紧急修复0.8.1的忘记执行shut的BUG
- 将load改名为init以避免含义冲突
- 完善reload功能，**警告：此操作不是线程安全**

#### 坏味道

- 重载模块是线程不安全的操作，考虑是否自行实现类RDMS表结构保存注册模块
...
- 新的归一化的@Component设计破坏了“包扫描是以子类为基准进行的扫描而非注解，设计目的是防止出现无实际代码的模块。”导致有可能出现无效模块，但是只影响扫描过程
- “去掉了模块必须继承构造方法的设计”，使用方法而非构造初始化模块底层内容，缺点是不能再用final
...
- 新增了插件包设定 动态加载plugin文件夹内的模块
- 去除了多种注解，统一为Component，由父类判断其类型而不是注解，简化开发
